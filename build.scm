    ...
    (if (null? args)
        ...
        (let ([cmd (car args)]
          ...
          (case (string->symbol cmd)
            ...
            [(module) (build-module args)]
            [(class) (build-class args)]
            [(syntax) (build-syntax args)]
            [(ftplugin) (build-ftplugin args)]
            [else (usage)])))))
...
(define (build-module args)
  ...
  (->> (list-ec [:let tsv (car args)]
       ...
       (filter-map (^[name] (rxmatch-case name
                              [else
                                #"syn keyword r7rsGaucheModName ~name"])))
       (for-each print))
  (exit))

(define (build-class args)
  (when (null? args)
    (usage "class TSV"
  ...
  (->> (list-ec [:let tsv (car args)]
                [:generator line (file->line-generator tsv)]
                [:let fields (string-split line #\tab)]
                [if (or (#/^{((meta|\w+ )?class)}$/ (~ fields 2))
                        (#/{condition type}/ (~ fields 2)))]
                (~ fields 3))
       (sort)
       (delete-duplicates)
       (map expand-@)
       ...
       (filter-map (^[name] (rxmatch-case name
                              [else
                                #"syn keyword r7rsGaucheClass ~name"])))
       (for-each print))
  (exit))

(define (build-syntax args)
  (when (null? args)
    (usage "syntax PATH [VIM...]"
           "Rebuild syntax/gauche.vim from generated vim files."
           "Args:"
           "PATH       path to syntax/gauche.vim"
           "VIM...     files generated by tsv command (macro|specialform|...)"))
  (let* ([path (car args)]
         [vim-files (cdr args)]
         [lines (file->string-list path)])
    (call-with-temporary-file
      (^[port tmp]
        (parameterize ([current-output-port port])
          (->> (list (->> lines
                          (take-while (^a (not (#/^" Common expressions {{{1$/ a))))
                          (map update-timestamp))
                     '("\" Common expressions {{{1" "")
                     (->> (list-ec [: file vim-files]
                     ...
                     '("")
                     (->> lines
                          (drop-while (^a (not (#/^" Special expressions {{{1$/ a))))))
               (concatenate)
               (for-each print))
          (flush))
        (copy-file tmp path :if-exists :supersede))))
  (exit))

(define (build-ftplugin args)
  (when (null? args)
    (usage "ftplugin PATH TSV"
  ...
  (let* ([path (car args)]
         [tsv (cadr args)]
         [lines (file->string-list path)]
         [lispwords (list-ec [:generator line (file->line-generator tsv)]
                             [:let fields (string-split line #\tab)]
                             [if (#/lispword/ (~ fields 4))]
                             (~ fields 3))])
    (call-with-temporary-file
      (^[port tmp]
        (parameterize ([current-output-port port])
          (->> (list (->> lines
                          (take-while (^a (not (#/^" lispwords {{{$/ a))))
                          (map update-timestamp))
                     '("\" lispwords {{{" "")
                     (->> lispwords
                          (sort)
                          (delete-duplicates)
                          (filter undefined-lispword?)
                          (filter-map (^[name] (rxmatch-case name
                                                 [else
                                                   #"setl lispwords+=~name"]))))
                     '("")
...
(define (update-timestamp line)
  (let1 today (date->string (current-date) "~Y-~m-~d")
    (regexp-replace-all #/(?<=Last Change:[[:space:]]*)[0-9]{4}-[0-9]{2}-[0-9]{2}/i line today)))
...
(define (undefined-lispword? word)
  (let* ([word (string-regexp-escape word)]
         [known? (string->regexp #"setl lispwords\\+?=(?:.+,)?~|word|(?:,|$)")])
    (not (any known? r7rs-lispwords))))
